...
async function test() {
  ...
  const customerId = "f5308a73-238e-4478-a937-9bc18c8d9ee9";
  const updatedData = new Customer();
  updatedData.email = "new@email.com";

  const result = await qpon.CUSTOMERS.updateCustomer(organizationId, customerId, updatedData);
  console.log(result);
}
