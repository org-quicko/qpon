...
import { Organization } from '@org.quicko.qpon/core';
...
async function test() {
  ...
  const updatedData = new Organization();
  updatedData.name = "Updated Organization Name";

  const result = await qpon.ORGANIZATIONS.updateOrganization(organizationId, updatedData);
  console.log(result);
}
